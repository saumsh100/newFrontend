...
describe('#Calculate total revenue for a date range', () => {
  ...
  test('For a date in the future calculate the estimated revenue, with closed clinic dates', async () => {
    await Appointment.bulkCreate([
      ...
      makeApptData({ patientId: patients[0].id, startDate: date(2018, 9, 8, 5), endDate: date(2018, 9, 8, 7), estimatedRevenue: 0 }),
    ]);
  ...
  test('For a date in the past calculate the total revenue based on delivered procedures only', async () => {
    ...
    await Appointment.bulkCreate([
      makeApptData({ patientId: patients[0].id, ...dates(2016, 7, 5, 8), estimatedRevenue: 500 }),
      makeApptData({ patientId: patients[0].id, ...dates(2016, 7, 4, 8), estimatedRevenue: 500 }),
      makeApptData({ patientId: patients[0].id, ...dates(2016, 7, 3, 8), estimatedRevenue: 500 }),
    ]);
  ...
  test('Not completed delivered procedures', async () => {
    ...
    await DeliveredProcedure.bulkCreate([
      ...
      makeProcedureData({ patientId: patients[1].id, entryDate: date(2016, 7, 4, 1), totalAmount: 300, isCompleted: true }),
      makeProcedureData({ patientId: patients[1].id, entryDate: date(2016, 7, 3, 10), totalAmount: 500 }),
    ]);
  ...
  test.skip('Date is between the start and end of the current day', async () => {
    ...
    const query = {
      date: date(currentYear, currentMonth, currentDay, 6),
      accountId,
      maxDates: 12,
      pastDaysLimit: 30,
    };
