...
export default function Patients(props) {
  const { fontColor, borderColor, data } = props;
  ...
  return (
    <Row className={styles.patients}>
      {data.map((d, index, items, array) => (
        <Col
          key={index}
          className={styles.patients__wrapper}
          xs={12}
          sm={Math.floor(12 / array)}
        >
          <Card className={styles.patients__item}>
            <div
              className={styles.patients__item_count}
            ...
            <Icon
              className={styles.patients__item_icon}
