...
export async function mapPatientsToRecalls({ recalls, account, startDate, endDate }) {
  ...
  for (const recall of recalls) {
    ...
    const patients = await exports.getPatientsForRecallTouchPoint({
      recall,
      account,
      startDate,
      endDate,
 ...
 */
export async function getPatientsForRecallTouchPoint({
  recall, account, startDate, endDate,
}) {
  /*
  ...
  */
  ...
  const start = moment(startDate)
    .add(recallIntervalObject)
    .subtract(recallBufferObject)
    .toISOString();
