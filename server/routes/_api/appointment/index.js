...
import { Appointment, AppointmentCode, Patient, sequelize } from 'CareCruModels';
import { sequelizeLoader } from '../../util/loaders';
import { fetchApptsWithCodes } from '../../../lib/queries/appointments';
import { allInsights } from '../../../lib/patientInsights/patientInformation';
...
appointmentsRouter.get('/', (req, res, next) => {
  ...
  const { limit, skip, filters = [], isParanoid } = query;
  ...
  const queryObj = {
    ...
    limit: parseInt(limitted, 10),
    include: includeArray,
    offset: parseInt(skipped, 10),
  };

  return Appointment.findAll(queryObj)
    .then((appointments) => {
      ...
      .then(async ({ appointment }) => {
        ...
        io &&
          io
            .of(ns)
            .in(accountId)
            .emit('CREATE:Appointment', appointment.id);
        ...
        return (
          ...
          io
            .of(ns)
            .in(accountId)
            .emit('create:Appointment', normalize('appointment', appointment))
        );
      })
      .catch(next);
  } catch (e) {
...
appointmentsRouter.put(
  ...
  (req, res, next) => {
    ...
    const appointmentUpdates = cleanedAppointments.map(appointment =>
      Appointment.findById(appointment.id).then(singleAppointment =>
        singleAppointment.update(appointment)));
...
appointmentsRouter.post(
  '/batch',
  checkPermissions('appointments:create'),
  checkIsArray('appointments'),
  async (req, res, next) => {
    const { appointments } = req.body;
    const cleanedAppointments = appointments.map(appointment =>
      Object.assign({}, omit(appointment, ['id']), { accountId: req.accountId }));
...
appointmentsRouter.put(
  ...
  (req, res, next) => {
    ...
    const appointmentUpdates = appointments.map(appointment =>
      Appointment.findById(appointment.id).then(_appointment => _appointment.update(appointment)));
...
appointmentsRouter.delete(
  '/batch',
  checkPermissions('appointments:delete'),
  async ({ query: { ids } }, res, next) => {
    try {
      console.log(ids);
      const appointmentsToDelete = ids
        .split(',')
        .map(id =>
          Appointment.findById(id).then(_appointment =>
            Promise.all([_appointment.update({ isDeleted: true }), _appointment.destroy()])));
...
if (globals.env !== 'production') {
  appointmentsRouter.get('/test', checkPermissions('appointments:read'), (req, res, next) => {
    ...
    return Appointment.filter(property)
      .run()
      .then((appointments) => {
...
appointmentsRouter.get('/:appointmentId', checkPermissions('appointments:read'), (req, res, next) =>
  Promise.resolve(req.appointment)
    .then(appointment =>
...
appointmentsRouter.put(
  ...
  async (req, res, next) => {
    ...
    const respondNormalized = (entity) => {
      const normalized = format(req, res, 'appointment', entity.get({ plain: true }));
      res.status(201).send(normalized);
      return { appointment: entity.dataValues };
    };

    return req.appointment
      .update(req.body)
      .then(respondNormalized)
      .then(async ({ appointment }) => {
        ...
        // This is assuming we won't get another PUT if isDeleted was already set, or else it's
        // gonna double send a DELETE event
        // We could probably catch this up top and throw a warning/error
        // DO NOT UPDATE AN APPOINTMENT W/ ISDELETED
        const action = appointment.isDeleted ? 'DELETE' : 'UPDATE';
        io &&
          io
            .of(ns)
            .in(accountId)
            .emit(`${action}:Appointment`, appointment.id);

        const pub = req.app.get('pub');
        pub.publish('APPOINTMENT:UPDATED', appointment.id);
        // TODO: why are we double sending? what was wrong with our current lowercase actions?
        // client-side is easy to update!
        return (
          io &&
          io
            .of(ns)
            .in(accountId)
            .emit('update:Appointment', normalize('appointment', appointment))
        );
      })
      .catch(next);
  },
...
appointmentsRouter.delete(
  '/:appointmentId',
  checkPermissions('appointments:delete'),
  async ({ app, accountId, appointment }, res, next) => {
    try {
      await Promise.all([appointment.update({ isDeleted: true }), appointment.destroy()]);
      res.sendStatus(204);
      ...
      return (
        ...
        io
          .of(ns)
          .in(accountId)
          .emit('remove:Appointment', normalized)
      );
    } catch (e) {
      next(e);
    }
