...
import {
  DailySchedule,
  Practitioner,
  // Disabling eslint on the next because the name is generated by the Class name
  Practitioner_Service, // eslint-disable-line camelcase
  Service,
  WeeklySchedule,
} from 'CareCruModels';
...
practitionersRouter.param(
  'practitionerId',
  sequelizeLoader('practitioner', 'Practitioner'),
);
 ...
 */
practitionersRouter.post(
  '/',
  checkPermissions('practitioners:create'),
  async (req, res, next) => {
    try {
 ...
 */
practitionersRouter.put(
  '/:practitionerId',
  checkPermissions('practitioners:update'),
  (req, res, next) => {
 ...
 */
practitionersRouter.put(
  '/:practitionerId/customSchedule',
  async (req, res, next) => {
    try {
      const {
        body: { isCustomSchedule },
        practitioner,
      } = req;
      if (isCustomSchedule) {
      ...
      return res
        .status(201)
        .send(normalize('practitioner', updatedPractitioner));
    } catch (err) {
      return next(err);
    }
  },
 ...
 */
practitionersRouter.post(
  '/:practitionerId/avatar',
  checkPermissions('practitioners:update'),
  async (req, res, next) => {
    const fileKey = `avatars/${req.practitioner.id}/${uuid.v4()}_[size]_${
      req.files.file.name
    }`;
...
practitionersRouter.delete(
  '/:practitionerId',
  checkPermissions('practitioners:delete'),
  (req, res, next) =>
    req.practitioner
      .destroy()
